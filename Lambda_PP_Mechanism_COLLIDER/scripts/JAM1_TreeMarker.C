...
void JAM1_TreeMarker(){
	...
	int N_DAT = 1000; // Convert N_DAT .dat file into one rootfile 
	int i_temp_dat = 0 ;
	...
	for(int i_file=0;i_file < Infiles.size();i_file++){
		...
		i_temp_dat ++; 
		if (i_temp_dat == N_DAT){
			TFile *fout = new TFile("/Users/feng/Documents/physics_project/Lambda_AuAu/genEvent/JAM1/forFeng2_SMode_root/JAM1_SMode_7p7.root","RECREATE");
			tree->Write();
			fout ->Close();
			delete fout;
		}
	...
	TFile *fout_stat = new TFile("/Users/feng/Documents/physics_project/Lambda_AuAu/genEvent/JAM1/forFeng2_SMode_root/stat.root","RECREATE");
